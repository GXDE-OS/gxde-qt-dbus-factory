    ...
    for (const QDBusIntrospection::Interface *interface : interfaces)
    {
        for (const auto method : interface->methods)
        {
            for (int i(0); i != method.outputArgs.size(); ++i)
            {
                ...
                if (QDBusMetaType::signatureToMetaType(arg.type.toLatin1()).id() != QVariant::Invalid)
        ...
        {
            if (QDBusMetaType::signatureToMetaType(property.type.toLatin1()).id() != QVariant::Invalid)
                continue;
